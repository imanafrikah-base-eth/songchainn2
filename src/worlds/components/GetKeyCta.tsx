...
// to the swap. Copy rule: access and belonging, never price.
...
export function GetKeyCta({
  ...
  const liveSwapUrl = tokenLive && world.swapUrl ? world.swapUrl : null;
  ...
  if (compact) {
    return liveSwapUrl ? (
      ...
      <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 px-4 py-2 text-xs font-semibold text-amber-300">
        <KeyRound className="h-3.5 w-3.5" /> The key is being cut. {world.tokenSymbol} goes live soon.
      </span>
    );
  }
