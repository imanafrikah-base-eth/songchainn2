...
async function liveContext(db: ReturnType<typeof admin>, token: string | null, page: string | null): Promise<string> {
  ...
  try {
    ...
    else lines.push("They are a listener, not an artist account (they can still become one from Studio).");
  } catch {
