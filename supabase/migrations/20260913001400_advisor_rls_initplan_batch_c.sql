-- Performance advisor: auth_rls_initplan, batch C (social_posts .. zabal_gamez_beat_downloads).
...
-- PERMISSIVE/RESTRICTIVE are untouched. The block self-verifies inside the
-- transaction and raises (rolling everything back) unless every policy on the
-- batch, once the wrapping is normalised away, reads exactly as before.
-- mosha_* tables and notifications are deliberately excluded (other agents).
do $mig$
declare
  tbls text[] := array['social_posts','song_analytics','song_purchases','songs','suggestion_forms','sync_requests','token_launches','upload_failures','user_blocks','user_follows','user_points','user_points_daily','user_roles','user_wallets','world_access_snapshots','world_citizens','world_episodes','world_feature_requests','world_meeting_requests','world_nfts','world_roles','world_tier_grants','world_voice_sessions','worlds','zabal_gamez_beat_downloads'];
